...
def validation(model, vilid_dataloader, criterion, scheduler, optimizer, epoch, IOU, valid_labels, number):
    model.eval()
    losses = AverageMeter()
...
def main():
    ...
    model_path = os.path.join(cfg.save_dir, cfg.exp_name,'textsegementation_best.pth'.format(cfg.backbone_name, cfg.checkepoch))
    # init or resume
    ...
    for epoch in range(start_epoch, cfg.max_epoch):
        ...
        IOU = validation(model, vilid_dataloader, criterion, scheduler, optimizer, epoch, IOU, valid_labels, number)
    print("final IOU: ", IOU)
    print('End.')
