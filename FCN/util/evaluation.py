...
def evaluation(image, output, one_mask, two_mask, all_mask, all_iou):
    for idx in range(image.size(0)):
        ...
        # one_pred_ = (np.array(one_pred) > cfg.confi_thresh).astype(np.uint8) * 1
        # two_pred_ = (np.array(two_pred) > cfg.confi_thresh).astype(np.uint8) * 1
        ...
        all_mask_ = all_mask[idx].cpu().numpy()
