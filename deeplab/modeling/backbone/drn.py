...
class DRN(nn.Module):
    ...
    def forward(self, x):
        if self.arch == 'C':
            ...
            layer0_x = self.layer0(x)

        layer1_x = self.layer1(layer0_x)
        layer2_x = self.layer2(layer1_x)

        layer3_x = self.layer3(layer2_x)
        low_level_feat = layer3_x

        layer4_x = self.layer4(layer3_x)
        layer5_x = self.layer5(layer4_x)

        if self.layer6 is not None:
            layer6_x = self.layer6(layer5_x)

        if self.layer7 is not None:
            layer7_x = self.layer7(layer6_x)

        if self.layer8 is not None:
            layer8_x = self.layer8(layer7_x)

        low_level_features = [layer1_x,layer2_x,layer3_x]
        return layer8_x, low_level_feat,low_level_features
